...
def startRegister(sessionId,id):
    ...
    body_ry = {"type":0,"timeout":60,"person":{"personId":id,"name": id}}
...
def trafficPerson_info(sessionId,id,cacheId,url):
    head = {"Content-Type": "application/json", "SessionId": sessionId}
    body_ry = {"person":{"empNo":id,"name":id,"personId":id,"remoteId":""},"face":[{"url":url,"cacheId":cacheId,"registerId":id}],"finger":[],"rule":[{"readerRule":{"ruleId":"DEVICEREADER"},"timezoneRule":{"ruleId":"TIMEZONERULE"}}]}
...
if __name__ == '__main__':
    ...
    while i<times:
        ...
        time.sleep(5)
        cacheId,url = registerStatus(sessionId)
        cancelRegister(sessionId)
        trafficPerson_info(sessionId, id, cacheId,url)
        i = i+1
